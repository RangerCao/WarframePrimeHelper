...
class WFPriceHelperApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Warframe 开核桃助手 [V5.3]")
...
程序启动后会自动同步市场缓存，代理变更后会自动重试同步。
    ...
    def process_screenshot(self):
        ...
        try:
                ...
                for dict_key in self.sorted_keys:
                    if dict_key in clean_ocr:
                        ...
                        if price_str:
                            ...
                            overlay_items.append((final_name, price_str, is_fast, set_price_str))
            ...
            for idx, (name, price, is_fast, sub_price) in enumerate(overlay_items):
                self.show_overlay(name, price, is_fast, index=idx, sub_content=sub_price)
