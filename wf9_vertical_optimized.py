...
class WFPriceHelperApp(ctk.CTk):
    ...
    def try_bow_part_fallback(self, base_url, real_name, clean_ocr):
        ...
        # 弓类优先尝试弓身，再到其他弓类部件
        suffix_candidates = ["grip", "string", "limb", "upper_limb", "lower_limb", "blueprint"]
        for suffix in suffix_candidates:
            test_url = f"{base_url}_{suffix}"
            test_price, test_is_fast = self.fetch_price_hybrid(test_url)
            if test_price:
                cn_name = SUFFIX_CN_NAME.get(suffix, suffix)
                final_name = f"{real_name} {cn_name}"
                self.log(f"   ↪ 弓类兜底命中: {cn_name}")
...
程序启动后会自动同步市场缓存，代理变更后会自动重试同步。
    ...
    def get_set_price_label(self, base_url):
    ...
    def build_ocr_candidates(self, ocr_result):
        ...
        for col in columns:
            ...
            # 额外窗口拼接：允许更长窗口，适配“弓身”掉到更下方多行的情况
            # 仅在同列内进行，不引入横向拼接。
            n = len(items)
            ...
            for clean_ocr in text_candidates:
                ...
                for dict_key in self.sorted_keys:
                    if dict_key in clean_ocr:
                        ...
                        # 部件未识别时，对弓类物品做兜底补查
                        if final_suffix == "set":
                            fb_suffix, fb_cn_name, fb_price, fb_is_fast = self.try_bow_part_fallback(base_url, real_name, clean_ocr)
                            if not fb_suffix:
                                continue
                            final_suffix = fb_suffix
                            cn_part_name = fb_cn_name
                            final_url = f"{base_url}_{final_suffix}"
                            final_name = f"{real_name} {cn_part_name}"
                            price_str, is_fast = fb_price, fb_is_fast
                        else:
                            final_url = f"{base_url}_{final_suffix}"
                            final_name = f"{real_name} {cn_part_name}"
                        ...
                        # 兜底1: 近战握柄有些使用 hilt 而不是 handle（如侍刃 Prime）
                        if (not price_str) and final_suffix == "handle":
                            alt_url_hilt = f"{base_url}_hilt"
                            alt_price_hilt, alt_is_fast_hilt = self.fetch_price_hybrid(alt_url_hilt)
                            if alt_price_hilt:
                                final_url = alt_url_hilt
                                final_suffix = "hilt"
                                final_name = f"{real_name} 握柄"
                                price_str, is_fast = alt_price_hilt, alt_is_fast_hilt
                                self.log("   ↪ 已回退为握柄(hilt)查询")

                        # 兜底2: 弓类部件在 OCR 下可能把 "弓身" 识别为 "握把"，
                        # 先命中 handle 会查不到，这里做 handle -> grip 兜底。
                        if (not price_str) and final_suffix == "handle":
                            alt_url = f"{base_url}_grip"
                            alt_price_str, alt_is_fast = self.fetch_price_hybrid(alt_url)
                            if alt_price_str:
                                final_url = alt_url
                                final_suffix = "grip"
                                final_name = f"{real_name} 弓身"
                                price_str, is_fast = alt_price_str, alt_is_fast
                                self.log("   ↪ 已回退为弓身(grip)查询")
                        set_price_str = self.get_set_price_label(base_url)
                        
                        if price_str:
                            self.log(f"   -> {price_str}")
                            seen_items.add(final_name)
                            found_count += 1
                            self.show_overlay(final_name, price_str, is_fast, index=overlay_index, sub_content=set_price_str)
                            overlay_index += 1
                        else:
                            self.log("   -> ❌ 未找到价格数据")
